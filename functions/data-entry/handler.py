...
BUCKET_NAMES = {"data-entry": "data-entry"}
...
def data_entry(client, chunk, index):
    # Upload data with content-type as application/csv
    csv = chunk.to_csv(index=False)
    client.put_object(
        BUCKET_NAMES["data_entry"],
        f"batch_{index}",
        io.BytesIO(bytes(csv, "ascii")),
        len(bytes(csv, "ascii")),
...
def handle():
    ...
    error = ""
    try:
        data_entry_bucket = BUCKET_NAMES["data-entry"]
    ...
    payload = (
        ...
        if error
            ...
            "success": True,
            "message": f"Entry data have been successfully uploaded to Minio in bucket {data_entry_bucket}",
        }
    )
