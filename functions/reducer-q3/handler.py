...
BUCKET_NAMES = {"reducer-q3": "reducer-data-q3", "final-q3": "final-results-q3"}
...
def sum_mapped_data(batch):
    payload = {"NE": 0, "NW": 0, "SE": 0, "SW": 0}
    for key, value in batch.items():
        sum = 0
        if value:
            sum = reduce(lambda cur, acc: cur + acc, value)
        payload.update({key: sum})
    return payload

def get_optimal_quarter(batch):
    sum_data = sum_mapped_data(batch)
    payload = {"optimal_quarter": max(sum_data, key=sum_data.get), "max_demand": 0}
...
def reducer(client, bucket, index, batch):
    ...
    client.put_object(
        bucket,
        f"Q3_reducer_results_batch_{index}",
...
def handle(req):
    ...
    payload = (
        ...
        if error
            ...
            "success": True,
            "message": f"Reducer data for q3 have been successfully uploaded to Minio in bucket {reducer_bucket}",
        }
    )
    return payload
